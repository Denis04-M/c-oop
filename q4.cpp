...
int main(){
	...
	for (auto itr = music.begin(); itr != music.end(); ++itr) {
	    ...
	    // storing the songs in a vector
    for (auto itr = music.begin(); itr != music.end(); ++itr) {
        songs.push_back(itr->second);
	}

	    // sorting and removing duplicate songs

    sort( songs.begin(), songs.end() );
    songs.erase( unique( songs.begin(), songs.end() ), songs.end() );

	  // printing unique sorted unique songs
	cout<<"\n"<<"Songs:"<<"\n";
    for (const string& i : songs) {
    cout << i << "  "<<"\n";
  }
	return 0;
}
